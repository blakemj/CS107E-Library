...
static int drawLine(char* str) {
    ...
    for (int charIndex = 0; charIndex < strlen(str); charIndex++) {
        ...
        if (str[charIndex] == '\n') {
            ...
            if (xCoord != numCharsDrawn) gl_draw_rect(xCoord * gl_get_char_width(), lineNum * font_get_height(), gl_get_char_width(), font_get_height(), BACK_COLOR);
            numCharsDrawn++;
            if (str[charIndex] != '\r') gl_draw_char(xCoord * gl_get_char_width(), lineNum * font_get_height(), str[charIndex], CHAR_COLOR);
            xCoord++;
        } 
    }
